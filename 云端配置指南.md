...
**服务器端（阿里云 ECS / Alibaba Cloud Linux 等）：**
```bash
# 安装
...
# 创建应用用到的用户和库（密码请自行替换为强密码）
sudo -u postgres psql -c "CREATE USER dnd_user WITH ENCRYPTED PASSWORD '你的密码';"
sudo -u postgres psql -c "CREATE DATABASE dnd_character_db OWNER dnd_user;"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE dnd_character_db TO dnd_user;"

# 允许本地密码认证：在 pg_hba.conf 中增加一行（在「host all all 127.0.0.1 ident」之上）
# host    dnd_character_db dnd_user  127.0.0.1/32    md5
# 然后：sudo systemctl reload postgresql
```

在部署目录的 `.env` 中配置：
`DATABASE_URL="postgresql://dnd_user:你的密码@localhost:5432/dnd_character_db?schema=public"`
并确保 systemd 服务已配置 `EnvironmentFile=/opt/charactersheet2024/current/.env`。部署后在该目录执行 `npx prisma migrate deploy` 创建表。
...
**服务器部署**：在部署目录（如 `/opt/charactersheet2024/current`）执行 `npx prisma migrate deploy` 即可，无需 `migrate dev`。

若同时部署**战役大厅**（dm-campaign-service），需在同一 PostgreSQL 实例上新建**独立库**（如 `dm_campaigns`），与角色卡库 `dnd_character_db` 分离；在战役大厅部署目录配置 `DATABASE_URL` 后执行 `npx prisma migrate deploy`。详见 [docs/战役大厅部署说明.md](docs/战役大厅部署说明.md) 第 4 节「数据库使用与服务器端配置」。
...
### 部署后 _next/static 或 admin 页面 404（ChunkLoadError）
- **原因**：部署后新构建的 chunk 文件名（hash）变了，但浏览器或 CDN 仍在使用旧的 HTML，请求的旧 chunk 已不存在。
- **用户立即解决**：在访问后台或出错页面时**强制刷新**（Windows/Linux：Ctrl+Shift+R，Mac：Cmd+Shift+R），或清除该站点缓存后重新打开。
- **长期缓解**：项目已在 `next.config.js` 中为页面设置 `Cache-Control: no-store, no-cache, must-revalidate`，部署后用户再次访问会拿到最新 HTML，减少此类 404。部署此配置后需重新发布一次生效。
...
## 11. 子域名部署（推荐）：https://cs.dimvision.xyz/

使用独立子域名 **cs.dimvision.xyz** 部署本应用，应用根路径即为 `/`，无需 basePath，可彻底避免子路径带来的白屏、静态资源 404 等问题。主站 https://dimvision.xyz/ 仍由 Nginx 指向其他项目，两个项目共用一台服务器、各自一个 server 块即可。

### 11.1 阿里云添加子域名解析

1. **登录阿里云控制台**  
   打开 [云解析 DNS 控制台](https://dns.console.aliyun.com/)（若域名在其它平台，则在该平台做解析）。

2. **找到主域名**  
   在「域名解析列表」中找到 **dimvision.xyz**，点击「解析设置」。

3. **添加解析记录**  
   - 点击「添加记录」  
   - **记录类型**：选 **A**（若服务器 IP 固定）或 **CNAME**（若用 CDN/负载均衡则选 CNAME 并填对应目标）。  
   - **主机记录**：填 **cs**（子域名为 cs.dimvision.xyz）。  
   - **记录值**：  
     - 选 A 时：填你的**服务器公网 IP**（与 dimvision.xyz 解析到的 IP 一致即可）。  
     - 选 CNAME 时：填提供的目标域名。  
   - **TTL**：默认 10 分钟即可。  
   - 保存。

4. **生效时间**  
   一般几分钟内生效，可用 `ping cs.dimvision.xyz` 检查是否解析到正确 IP。

### 11.2 服务器 Nginx 配置（子域名）

在**同一台服务器**的 Nginx 中新增一个 `server` 块，专门处理 cs.dimvision.xyz（与主站 dimvision.xyz 的 server 块并列）：

```nginx
# 角色表应用：子域名 cs.dimvision.xyz（推荐，无 basePath）
server {
    listen 80;
    server_name cs.dimvision.xyz;
...
说明：本应用监听 **3100** 端口，请与 systemd 中 `charactersheet2024` 的 `-p 3100` 一致。若你使用其它端口，把 `3100` 改成实际端口。
...
### 11.3 为子域名配置 HTTPS（推荐）

- **若主站已用 Let’s Encrypt（如 certbot）**：为子域名单独申请证书，例如：
  ```bash
  sudo certbot --nginx -d cs.dimvision.xyz
  ```
  certbot 会自动在 Nginx 里加上 `listen 443 ssl` 并配置证书路径。

- **若主站用阿里云 SSL 证书**：  
  1. 在 [SSL 证书控制台](https://yundun.console.aliyun.com/?p=cas) 申请或绑定证书（可申请免费 DV 证书，支持单域名或泛域名 *.dimvision.xyz）。  
  2. 若使用单域名证书，需为 **cs.dimvision.xyz** 单独申请/绑定一个证书。  
  3. 在 Nginx 中为 `server_name cs.dimvision.xyz` 的 server 块增加 `listen 443 ssl` 及 `ssl_certificate` / `ssl_certificate_key` 配置（可参考主站 dimvision.xyz 的 443 写法）。

配置完成后，访问 **https://cs.dimvision.xyz/** 即可使用本应用。

### 11.4 部署时使用子域名（无 basePath）

部署脚本**默认已按子域名**：不设置 `BASE_PATH` 时，构建出的应用根路径为 `/`。
...
如需临时改回子路径部署（如仍用 dimvision.xyz/charactersheet2024），再执行：

```bash
BASE_PATH=/charactersheet2024 bash scripts/deploy-aliyun.sh
```

---

## 12. 子路径部署 Nginx 配置（备用：dimvision.xyz/charactersheet2024）

若仍使用子路径（如 `https://dimvision.xyz/charactersheet2024/`），**必须**把完整路径转发给 Next.js，否则会出现「文档请求 200 但 0 kB、页面白屏」。
